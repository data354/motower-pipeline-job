...
def gen_motower_daily(**kwargs):
    ...
    if not data.empty:  
        ...
        print('---------------------------------------------------------')
        print(data["jour"].unique()) 
        print('---------------------------------------------------------')
        write_pg(PG_SAVE_HOST, PG_SAVE_DB, PG_SAVE_USER, PG_SAVE_PASSWORD, df_dimension, "motower_daily_caparc_dimension_new") 
        write_pg(PG_SAVE_HOST, PG_SAVE_DB, PG_SAVE_USER, PG_SAVE_PASSWORD, df_faits, "motower_daily_caparc_faits_new") 
    else:
        raise RuntimeError(f"No data for {kwargs['ingest_date']}")
