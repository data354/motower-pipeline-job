...
with DAG(
    ...
    start_date=datetime(2023, 1, 3, 6, 30, 0),
    end_date= datetime(2023, 2, 1, 6, 30, 0),
    catchup=True
) as dag:
    ...
    ingest_hdrp = PythonOperator(
        ...
        op_kwargs={'thetable': CONFIG["tables"][1]["name"],
                   'bucket': CONFIG["tables"][1]["bucket"],
                   'folder': CONFIG["tables"][1]["folder"],
    ...
    ingest_ftp = PythonOperator(
        task_id='ingest_ftp',
        provide_context=True,
        python_callable=extract_a_ftp_job,
