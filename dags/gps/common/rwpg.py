...
def write_pg(host: str, database:str, user: str, password: str,
    ...
    if table == "motower_weekly":
        exec_date = data["jour"].unique()[0]
        delete_query = f"DELETE FROM {table} WHERE EXTRACT(WEEK FROM jour) = {exec_date.astype(dt).isocalendar()[1]} and EXTRACT(YEAR FROM jour) = {exec_date.astype(dt).isocalendar()[0]} "
    if table == "motower_monthly":
        #delete_query = "DELETE FROM "+table+ " WHERE mois = %s;", (data.mois.unique()[0],)
